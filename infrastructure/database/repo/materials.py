...
class MaterialRepo(BaseRepo):
    ...
    async def update_materials(self, subject: str, images: list[str]):
        stmt = insert(SubjectMaterial).values(
            subject=subject,
            images=images
    ...
    async def clear_materials(self, subject: str):
        stmt = update(SubjectMaterial).where(SubjectMaterial.subject == subject).values(images=[])
        await self.session.execute(stmt)
        await self.session.commit()
