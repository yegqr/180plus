...
async def main():
    ...
    async def process_one(q_id, subject, q_num, explanation, year, session_name, session_pool):
        async with semaphore:
            async with session_pool() as session:
                ...
                if not explanation or len(explanation.strip()) < 5:
                    logger.warning(f"{subject} | {year} | {session_name} | {q_num} | - | - | - = ⚠️ SKIPPED (No explanation)")
                    return

                try:
                    # 5. Ask Gemini
                    prompt = (
                        "Analyze NMT History exam question explanation.\n"
                        "Extract the correct answer from the explanation text.\n"
    ...
    async with session_pool() as session:
        ...
        logger.info(f"🔍 Auditing {len(questions)} history questions from explanations (2 workers)...")
        tasks_data = [(q.id, q.subject, q.q_number, q.explanation, q.year, q.session) for q in questions]
