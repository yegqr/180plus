...
async def get_calculator_data(dialog_manager: DialogManager, **kwargs):
    ...
    final_kb = 0.0
    k_vals = {"k1": 0, "k2": 0, "k3": 0, "k4": 0, "kt": 0, "k4max": 0}
    budget_status = "❌"
    
    if spec:
        k_vals["k1"] = spec["main_block"].get("ukr_mova", 0)
        k_vals["k2"] = spec["main_block"].get("ukr_history", 0)
        k_vals["k3"] = spec["main_block"].get("math", 0)
        
        if fourth_subj_id:
            k_vals["k4"] = spec["choosable_block"].get(fourth_subj_id, 0)
        
        k_vals["k4max"] = max(spec["choosable_block"].values()) if spec["choosable_block"] else 0
        k_vals["kt"] = spec.get("tvorchy_konkurs") or 0
    ...
    hint = "Введіть бал:"
    if current_input in ["btn_p1", "btn_p2", "btn_p3", "btn_p4"]:
        subj_mapping = {
            "btn_p1": ("ukr_mova", "Укр. мови"),
            "btn_p2": ("ukr_history", "Історії"),
            "btn_p3": ("math", "Математики"),
            "btn_p4": (fourth_subj_id, fourth_subj_name)
        }
        subj_id, display_name = subj_mapping.get(current_input, (None, ""))
    ...
    elif current_input == "btn_tk":
        hint = "Введіть бал за <b>Творчий конкурс</b> (100-200):"
    ...
    return {
        ...
        "formula": f"<code>({k_vals['k1']}*{ratings['ukr_mova']} + {k_vals['k2']}*{ratings['ukr_history']} + {k_vals['k3']}*{ratings['math']} + {k_vals['k4']}*{ratings['fourth_subj']} + {k_vals['kt']}*{ratings['tvorch']})/({k_vals['k1'] + k_vals['k2'] + k_vals['k3'] + (k_vals['k4max'] + k_vals['k4'])/2 + k_vals['kt']:.1f}) * {region['coef']} * {gk} = {final_kb:.3f}</code>" if spec else ""
    }

async def save_calc_data(dialog_manager: DialogManager, update_dict: dict):
...
async def on_spec_selected(callback: CallbackQuery, widget: Any, dialog_manager: DialogManager, item_id: str):
    await save_calc_data(dialog_manager, {"spec_code": item_id})
    await dialog_manager.switch_to(CalculatorSG.main)

async def on_region_selected(callback: CallbackQuery, widget: Any, dialog_manager: DialogManager, item_id: str):
    await save_calc_data(dialog_manager, {"region_id": int(item_id)})
    await dialog_manager.switch_to(CalculatorSG.main)

async def on_courses_toggle(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    user: User = dialog_manager.middleware_data.get("user")
    current = user.settings.get("calc", {}).get("courses_active", False)
    await save_calc_data(dialog_manager, {"courses_active": not current})

async def on_reset(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    repo: RequestsRepo = dialog_manager.middleware_data.get("repo")
    user: User = dialog_manager.middleware_data.get("user")
    user.settings["calc"] = {}
    await repo.users.update_user_settings(user.user_id, user.settings)
...
async def on_input_points(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    dialog_manager.dialog_data["current_input"] = button.widget_id
    await dialog_manager.switch_to(CalculatorSG.input_points)

async def on_points_entered(message: Message, widget: Any, dialog_manager: DialogManager):
    user: User = dialog_manager.middleware_data.get("user")
...
async def on_spec_search(message: Message, widget: Any, dialog_manager: DialogManager):
...
async def on_fourth_subj_selected(callback: CallbackQuery, widget: Any, dialog_manager: DialogManager, item_id: str):
    await save_calc_data(dialog_manager, {"fourth_subj_id": item_id})
    await dialog_manager.switch_to(CalculatorSG.main)

async def on_region_btn_click(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    await dialog_manager.switch_to(CalculatorSG.select_region)

async def on_search_btn_click(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    await dialog_manager.switch_to(CalculatorSG.search_spec)

async def on_fourth_subject_btn_click(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    await dialog_manager.switch_to(CalculatorSG.select_fourth_subject)

async def on_back_to_main(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    await dialog_manager.switch_to(CalculatorSG.main)

async def on_ask_kse_btn_click(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    await dialog_manager.switch_to(CalculatorSG.ask_kse)

async def on_kse_question_sent(message: Message, widget: Any, dialog_manager: DialogManager):
    user: User = dialog_manager.middleware_data.get("user")
    repo: RequestsRepo = dialog_manager.middleware_data.get("repo")
...
async def get_subjects_data(dialog_manager: DialogManager, **kwargs):
    return {"subjects": [{"id": k, "name": v} for k, v in CHOOSABLE_SUBJECT_NAMES.items()]}

async def get_regions_data(dialog_manager: DialogManager, **kwargs):
